...
class MockInputOutputManager(InputOutputManager):
    ...
    def get_run_numbering(self):
        return self.get_run_numbering_reversed()

    def get_run_numbering_reversed(self):
        self.run_numbering = {num: image_info for num, image_info in self.run_numbering.items() if image_info.image_name not in self.msg["error"]}    
        sorted_keys = sorted(self.run_numbering, reverse=True)
