...
TEST_ALL_MODELS = True  # Whether to test all models or only those marked as image-capable
...
def test_models():
    ...
    if TEST_ALL_MODELS:
        ...
        models_to_test = [model for model in all_models if BedrockImageProcessor.is_model_image_capable(model)]
    ...
    for i, model_id in enumerate(models_to_test):
        ...
        try:
            ...
            result = {
                ...
                "tokens": {
                    "input": processing_data.get("input tokens", 0),
                    "output": processing_data.get("output tokens", 0),
                    "total": processing_data.get("input tokens", 0) + processing_data.get("output tokens", 0)
                },
                "costs": {
                    "input": processing_data.get("input cost $", 0),
                    "output": processing_data.get("output cost $", 0),
                    "total": processing_data.get("input cost $", 0) + processing_data.get("output cost $", 0)
                }
            }
    ...
    if successful_tests:
        test_results["summary"] = {
            ...
            "fastest_model": min(successful_tests, key=lambda x: x["processing_time_seconds"])["model_id"],
            "slowest_model": max(successful_tests, key=lambda x: x["processing_time_seconds"])["model_id"],
            "cheapest_model": min(successful_tests, key=lambda x: x["costs"]["total"])["model_id"],
            "most_expensive_model": max(successful_tests, key=lambda x: x["costs"]["total"])["model_id"],
            "average_time": sum(r["processing_time_seconds"] for r in successful_tests) / len(successful_tests),
            "average_cost": sum(r["costs"]["total"] for r in successful_tests) / len(successful_tests),
            "total_cost": sum(r["costs"]["total"] for r in successful_tests)
        }
